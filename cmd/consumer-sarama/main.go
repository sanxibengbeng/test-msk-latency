...
func main() {
	...
	log := logger.NewLogger("ConsumerSaramaMain")
	log.Info("Starting Sarama consumer in %s environment", *envFlag)
	...
	log.Info("Starting %d Sarama consumers", consumerCount)

	for i := 0; i < consumerCount; i++ {
		wg.Add(1)
		consumer, err := kafka.NewConsumerSarama(cfg, i+1)
		if err != nil {
			log.Error("Failed to create Sarama consumer %d: %v", i+1, err)
			continue
		}
	...
	wg.Wait()
	log.Info("All Sarama consumers have shut down")
}
